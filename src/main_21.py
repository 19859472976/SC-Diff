...
from model_21 import create_model_diffu, Att_Diffuse_model,EnhancedAttDiffuseModel
...
def test(args, model, model_name,
         ...
         test_list,test_data,
         use_cuda,num_nodes, num_rels, entity_dict, rel_dict, mode='eval',ts= time.asctime(),epoch=30,static_graph=None):
    """
    ...
    with torch.no_grad():
        for time_idx, test_snap in enumerate(tqdm(test_list,colour='RED')):
            ...
            history_glist = [] # choose train data with time
            ...
            sequence = np.load('./data/{}/{}/h_r_seen_triple_{}.npy'.format(args.dataset, test_data, str(query_time))
            ...
            if args.add_static_graph:
                all_tail_seq = sp.load_npz('./data/{}/{}/h_r_history_seq_{}.npz'.format(args.dataset, test_data, str(query_time)))  # 统计历史事实是否出现
...
#                batch_x = sequence[:, :original_length - args.max_len, :].float().to(device)
#                batch_y = sequence[:, -args.max_len:, :].float()
#                batch_x_mark = torch.ones_like(batch_x).float().to(device)
#                dec_inp = torch.zeros_like(batch_y[:, -args.pred_len:, :]).float()
#                dec_inp = torch.cat([batch_y[:, :args.label_len, :], dec_inp], dim=1).float().to(device)
#                batch_y_mark = torch.ones_like(batch_y).float().to(device)
#                _, y_0_hat_batch, KL_loss, z_sample = cond_pred_model(batch_x, batch_x_mark, dec_inp,
#                                                                              batch_y_mark)
                                                                              
                scores, diffu_rep, weights, t, _,ent_emb,KL_loss = model(history_glist, sequence,true_triples, entity_dict, rel_dict, False, use_cuda
                                                                 ,seen_before_label = seen_before_label,static_graph=static_graph)
...
def getReverseSample(test_triplets,num_rels):
    inverse_test_triplets = test_triplets[:, [2, 1, 0, 3]] # inverse triplets 
...
def annealing_schedule(cur_,T0,TE):
...
def run_experiment(args):
    ...
    entity_dict = utils.read_dict(args.dataset, "entity2id.txt")
    rel_dict = utils.read_dict(args.dataset, "relation2id.txt")
...
#                 num_bases=args.n_bases)
    model = EnhancedAttDiffuseModel(diffu_rec, args,args.encoder,
                 num_nodes,
                 num_rels,
                 dropout=args.dropout,
                 max_time = len(history_times),
...
#        graph_dict = pickle.load(fp)
    ...
    if args.test:
        ...
        print(test_filter_mrr_obj,test_mrr_obj,test_filter_hit_obj,test_hit_obj)
...
#        optim.Adam(cond_pred_model.parameters(), lr=0.001)
        for epoch in range(args.n_epochs):
            ...
            for batch_idx, train_sample_num in enumerate(tqdm(idx)):
                ...
                sequence = np.load('./data/{}/test_seq/h_r_seen_triple_{}.npy'.format(args.dataset,str(train_times[train_sample_num]))
                ...
                history_glist = []  # [(g), ...], len = valid history length
                ...
                seq_idx = true_triples[:, 0] * num_rels * 2 + true_triples[:, 1] 
                if args.add_static_graph:
                    all_tail_seq = sp.load_npz('./data/{}/test_seq/h_r_history_seq_{}.npz'.format(args.dataset, str(train_times[train_sample_num-1])))  # 统计历史事实是否出现
                    history_tail_seq = torch.Tensor(all_tail_seq[seq_idx.cpu()].todense())
                    one_hot_tail_seq = history_tail_seq.masked_fill(history_tail_seq != 0, 1)
                    if use_cuda:
                        history_tail_seq, one_hot_tail_seq = history_tail_seq.to(args.gpu), one_hot_tail_seq.to(args.gpu)
                ...
                scores, diffu_rep, weights, t, mask_seq,emb_ent, KL_loss = model(history_glist, sequence, true_triples, epoch, entity_dict, rel_dict, train_flag=True, use_cuda=use_cuda,seen_before_label=seen_before_label, static_graph=static_graph)
                loss_diffu_value = model.loss_diffu_ce(diffu_rep, true_triples[:,2],history_tail_seq, one_hot_tail_seq,true_triples,mask_seq,emb_ent) + KL_loss
            ...
            if (epoch + 1) and (epoch + 1) % args.evaluate_every == 0:
                ...
                print(dev_mrr_obj,dev_hit_obj,dev_filter_mrr_obj,dev_filter_hit_obj)
                print(test_mrr_obj,test_hit_obj,test_filter_mrr_obj,test_filter_hit_obj)
                if dev_filter_mrr_obj < best_val_mrr:
                    ...
                    best_test_hit = test_filter_hit_obj
                    best_epoch = epoch
                    torch.save({'state_dict': model.state_dict(), 'epoch': epoch}, model_state_file)
            print("Epoch {:04d}, AveLoss: {:.4f}, BestValMRR {:.4f}, BestTestMRR: {:.4f}, test_hit@1: {:.4f}, test_hit@3: {:.4f}, test_hit@10: {:.4f}, Model: {}, Dataset: {} "
                  .format(epoch, np.mean(losses), best_val_mrr, best_test_mrr, best_test_hit[0], best_test_hit[1], best_test_hit[2], model_name, args.dataset))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--add_static",action='store_true', default=False)
    ...
    parser.add_argument('--label_len', type=int, default=63, help='start token length')
    ...
    parser.add_argument('--distil', action='store_false',
                        ...
                        default=True)
    parser.add_argument('--forecasting_dropout', type=float, default=0.05, help='dropout')
