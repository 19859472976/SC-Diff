...
#    mkdirs(save_dir_obj)
    ...
    sp.save_npz('./data/{}/train_seq/h_r_seq_rel.npz'.format(dataset), tail_rel)
    sp.save_npz('./data/{}/test_seq/h_r_seq_rel.npz'.format(dataset), tail_rel)
    ...
    sp.save_npz('./data/{}/train_seq/h_r_history_train_valid.npz'.format(dataset), tail_all)
    sp.save_npz('./data/{}/test_seq/h_r_history_train_valid.npz'.format(dataset), tail_all)
    ...
    sp.save_npz('./data/{}/train_seq/h_r_history_train.npz'.format(dataset), tail_train)
    sp.save_npz('./data/{}/test_seq/h_r_history_train.npz'.format(dataset), tail_train)
    tail_seq = sp.csr_matrix(([], ([], [])), shape=(num_e * num_r_2, num_e)) 
    sp.save_npz('./data/{}/train_seq/h_r_history_seq_{}.npz'.format(dataset, all_times[0]), tail_seq)
    sp.save_npz('./data/{}/test_seq/h_r_history_seq_{}.npz'.format(dataset, all_times[0]), tail_seq)
    for idx, tim in tqdm(enumerate(all_times)):  
        ...
        sp.save_npz('./data/{}/train_seq/h_r_history_seq_{}.npz'.format(dataset, all_times[idx]), tail_seq)
        sp.save_npz('./data/{}/test_seq/h_r_history_seq_{}.npz'.format(dataset, all_times[idx]), tail_seq)
...
def main_fun():     
    for dataset in ['ICEWS14','ICEWS18','ICEWS05_15','GDELT']: #'ICEWS14',
        get_freq(dataset)
