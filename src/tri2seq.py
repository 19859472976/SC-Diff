...
def process_triples(matrix, data_list ,max_sequence_length=-1,m=-1,r=-1,history_times=-1):
    t = len(matrix)
#    sequences_dict = np.full((m,max_sequence_length,4),-1,dtype=int)
    ...
    for ti in trange(0,t):
        inverse_test_triplets = matrix[ti][:, [2, 1, 0, 3]] # inverse triplets 
        inverse_test_triplets[:, 1] = inverse_test_triplets[:, 1] + r
        matrix[ti] = np.concatenate((matrix[ti], inverse_test_triplets))
    for ti in trange(0,t):
        if ti == 0:
            np.save('./data/{}/{}/h_r_seen_triple_{}.npy'.format(dataset, data_list, str(history_times[ti])), sparse.csr_matrix((sequences_dict+1)[matrix[ti][:,0]].reshape(len(matrix[ti]),-1)))
            continue
...
#            current_sequence = np.insert(current_sequence, len(current_sequence), np.array([o, r, ti-1],int), axis=0)
        ...
        np.save('./data/{}/{}/h_r_seen_triple_{}'.format(dataset, data_list, str(history_times[ti])), sparse.csr_matrix((sequences_dict+1)[matrix[ti][:,0]].reshape(len(matrix[ti]),-1)))
for dataset in ['ICEWS14']: #'GDELT',,'ICEWS05-15'
...
#    history_times = np.concatenate((train_times, val_times), axis=None) #, test_times
    train_list.extend(valid_list)
    train_list.extend(test_list)
#    process_triples(train_list, 'train_seq',max_sequence_length=128,m=data.num_nodes,r=data.num_rels,history_times=history_times)
    history_times = np.concatenate((train_times, val_times, test_times), axis=None) #, test_times
#    train_list.extend(test_list)
    process_triples(train_list, 'train_seq', max_sequence_length=128, m=data.num_nodes, r=data.num_rels,
